...
bool ReallocateBottleneckPLink(const SASolution* prev_sol,
  ...
  int bneck_u = plink_it->bneck_edge.first, bneck_v = plink_it->bneck_edge.second;
  ...
  while (new_sol->util_matrix.matrix[bneck_u][bneck_v] > 
    ...
    for (vlink_it = mapped_vlinks.begin(); vlink_it != mapped_vlinks.end();
      ...
      DEBUG("vn = %d, vlink (%d, %d), n_bottlenecks = %d\n", 
          vlink_it->first, vlink_it->second.first, vlink_it->second.second,
          n_bottlenecks);
    ...
    if (selected_vlink.first != NIL) {
      ...
      UpdateReverseLinkEmbedding(
          new_sol->r_embedding.get(), selected_vlink, old_path, *embedded_path);
      ...
      DEBUG("After vlink reallocation, embedded vlinks on plink (%d, %d) are:\n",
            bneck_u, bneck_v);
      const vedge_set_t& vet = 
        new_sol->r_embedding->redge_map.find(edge_t(bneck_u, bneck_v))->second;
      vedge_set_t::const_iterator vit;
      for (vit = vet.begin(); vit != vet.end(); ++vit) {
          DEBUG("Vlink (%d, %d) from vn %d\n", 
              vit->second.first, vit->second.second, vit->first);
      }
    } else {
      break;
    }
...
bool PerformVLinkMigration(const SASolution* current_solution, 
  ...
  while (num_migration_attempt++ < num_migration_candidates) {
    ...
    int vlink_u = vlp.vn_edge.second.first,
        vlink_v = vlp.vn_edge.second.second;
    ...
    int ph_dest =
        neighbor_solution->vn_embeddings[vn_index].node_map[vlink_v];
    ...
    UpdateUtilMatrix(neighbor_solution->physical_topology,
      neighbor_solution->util_matrix, neighbor_solution->res_bw_matrix, old_path);
    ...
    if (embedded_path->size() > 0) {
      ...
      UpdateResidualBandwidth(neighbor_solution->res_bw_matrix,
                              *embedded_path, -bw);
      UpdateUtilMatrix(
          neighbor_solution->physical_topology,
          neighbor_solution->util_matrix, neighbor_solution->res_bw_matrix,
          *embedded_path);
      std::vector<vn_edge_t> vlinks_changed(1, vlp.vn_edge);
      std::vector<path_t> old_paths(1, old_path);
      std::vector<path_t> new_paths(1, *embedded_path);
      double cost_delta = GetCostDelta(neighbor_solution,
                                     vlinks_changed, old_paths, new_paths);
...
bool PerformNodeMigration(const SASolution* current_solution,
  ...
  for (int i = 0;
    ...
    const std::vector<edge_endpoint>& neighbors =
        neighbor_solution->virt_topologies[vn_index].adj_list()->at(
            vnode_u);
    for (vend_point_it = neighbors.begin();
      ...
      embedded_paths.push_back(ConstrainedVLinkEmbed(
          neighbor_solution->physical_topology,
          neighbor_solution->util_matrix, neighbor_solution->res_bw_matrix,
          temp_u_map, vnode_v_map, bw, std::set<edge_t>()).release());
      ...
      total_path_cost +=
          GetEmbeddedPathCost(vn_edge_t(vn_index, ConstructEdge(vnode_u, vnode_v)),
                              recent_path, neighbor_solution);
    }
  ...
  if (best_u_map != NIL) {
    DEBUG("Reallocating vnode %d of vn %d from pnode %d to pnode %d\n", 
          vnode_u, vn_index, vnode_u_map, best_u_map);
    const std::vector<edge_endpoint>& neighbors =
        neighbor_solution->virt_topologies[vn_index].adj_list()->at(
            vnode_u);
    std::vector<edge_endpoint>::const_iterator vend_point_it;
    ...
    for (vend_point_it = neighbors.begin();
      ...
      for (int x = 0; x < embedded_path->size(); ++x) {
        DEBUG("[new_path]: (%d, %d)\n", embedded_path->at(x).first, embedded_path->at(x).second);
      }
      ...
      UpdateUtilMatrix(neighbor_solution->physical_topology,
          ...
          *embedded_path);
      UpdateReverseLinkEmbedding(
          neighbor_solution->r_embedding.get(),
          vl, old_path,
          *embedded_path);
      old_path.assign(embedded_path->begin(), embedded_path->end());
      neighbor_solution->vlinks_by_plength.update(
    ...
    UpdateReverseNodeEmbedding(neighbor_solution->r_embedding.get(),
                               ...
                               best_u_map);
    double cost_delta = GetCostDelta(
        neighbor_solution, changed_vlinks, old_paths, new_paths);
    DEBUG("Old cost = %lf, cost_delta = %lf\n", neighbor_solution->cost, cost_delta);
    neighbor_solution->cost += cost_delta;
    // Update bookkeeping information.
    neighbor_solution->node_bw_heap_handlers[nbw.vnode] = 
...
unique_ptr<SASolution> GenerateNeighbor(const SASolution& current_solution) {
  ...
  enum {ALTER_BNECK = 0, NODE_MIGRATE, LINK_MIGRATE};
  static boost::random::mt19937 generator;
  int n_possible_operations = 3;
  static boost::random::uniform_int_distribution<> dist(0, n_possible_operations - 1);
  ...
  switch(current_operation) {
    case ALTER_BNECK: 
      total_bottlenecks = 
        neighbor_solution->bottleneck_edges.size();
